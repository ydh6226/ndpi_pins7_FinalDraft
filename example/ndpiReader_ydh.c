...
static void printFlow(u_int16_t id, struct ndpi_flow_info *flow, u_int16_t thread_id)
{
  ...
  if (csv_fp != NULL)
  {
    ...
    double benign_score = dos_ge_score < 1 && dos_slow_score < 1 &&
                                  ...
                                  ftp_patator_score < 1 && ssh_patator_score < 1 && inf_score < 1
                              ? 1.1
                              : 0;
  ...
  if (flow->ssh_tls.ssl_version != 0)
    fprintf(out, "[%s]", ndpi_ssl_version2str(flow->ssh_tls.ssl_version, &known_tls));
  if (flow->ssh_tls.client_info[0] != '\0')
    fprintf(out, "[Client: %s]", flow->ssh_tls.client_info);
  ...
  if ((flow->detected_protocol.master_protocol == NDPI_PROTOCOL_TLS) || (flow->detected_protocol.app_protocol == NDPI_PROTOCOL_TLS))
  {
    if ((flow->ssh_tls.sha1_cert_fingerprint[0] == 0) && (flow->ssh_tls.sha1_cert_fingerprint[1] == 0) && (flow->ssh_tls.sha1_cert_fingerprint[2] == 0))
      ; /* Looks empty */
    else
    {
  ...
  if (flow->ssh_tls.server_cipher != '\0')
    fprintf(out, "[Cipher: %s]", ndpi_cipher2str(flow->ssh_tls.server_cipher));
                                            ...
                                            ndpi_VISIT which, int depth, void *user_data)
{
  ...
  if ((flow->detected_protocol.master_protocol != NDPI_PROTOCOL_UNKNOWN) || (flow->detected_protocol.app_protocol != NDPI_PROTOCOL_UNKNOWN))
    return;
                 ...
                 addr_node **vrootp, const char *proto)
{
  ...
  while (*rootp != (addr_node *)0)
  {
    ...
    rootp = (key < (*rootp)->addr) ? &(*rootp)->left : /* T3: follow left branch */
                            ...
                            struct receiver **topRcvrs)
{
  ...
  if (r == NULL)
  {
    if (((size = HASH_COUNT(*rcvrs)) < MAX_TABLE_SIZE_1) || ((a = acceptable(num_pkts)) != 0))
    {
...
static void port_stats_walker(const void *node, ndpi_VISIT which, int depth, void *user_data)
{
  ...
  { /* Avoid walking the same node multiple times */
    ...
    if (flow->detected_protocol.master_protocol)
      ...
      strncpy(proto, ndpi_get_proto_name(ndpi_thread_info[thread_id].workflow->ndpi_struct, flow->detected_protocol.app_protocol), sizeof(proto));

    if (((r = strcmp(ipProto2Name(flow->protocol), "TCP")) == 0) && (flow->src2dst_packets == 1) && (flow->dst2src_packets == 0))
    {
...
static void printResults(u_int64_t processing_time_usec, u_int64_t setup_time_usec)
{
  ...
  for (thread_id = 0; thread_id < num_threads; thread_id++)
  {
    if ((ndpi_thread_info[thread_id].workflow->stats.total_wire_bytes == 0) && (ndpi_thread_info[thread_id].workflow->stats.raw_packet_count == 0))
      continue;
...
static pcap_t *openPcapFileOrDevice(u_int16_t thread_id, const u_char *pcap_file)
{
                                    ...
                                    promisc, 500, pcap_error_buffer)) == NULL)
  {
    ...
    if ((pcap_handle = pcap_open_offline((char *)pcap_file, pcap_error_buffer)) == NULL)
    {
      ...
      else if ((getNextPcapFileFromPlaylist(thread_id, filename, sizeof(filename)) != 0) || ((pcap_handle = pcap_open_offline(filename, pcap_error_buffer)) == NULL))
      {
                                ...
                                const u_char *packet)
{
  ...
  if (packet_checked[modbus_header_offset + 11] == 0x05 && packet_checked[modbus_header_offset + 12] == 0xde && packet_checked[modbus_header_offset + 23] == 0x80 && packet_checked[modbus_header_offset + 24] == 0x18)
  {
    modbus_src = 1;
    modbus_dst = 0;
  }
  if (packet_checked[modbus_header_offset + 13] == 0x05 && packet_checked[modbus_header_offset + 14] == 0xde && packet_checked[modbus_header_offset + 23] == 0x80 && packet_checked[modbus_header_offset + 24] == 0x18)
  {
    modbus_src = 0;
    modbus_dst = 1;
  }
  if (packet_checked[modbus_header_offset] == 0x06 && (modbus_src + modbus_dst) == 1)
  {
    ...
    fprintf(fp2, "========== Modbus packet count: %d num: %d ==========\n\n", modbus_count, all_count);
      ...
      packet_checked[dnp_offset + 1] == 0x64)
  {
    ...
    fprintf(fp1, "========== DNP 3.0 packet count: %d num: %d ==========\n\n", dnp_count, all_count);
      ...
      packet_checked[payload_offset + 7] == 0x32)
  {
    ...
    if (packet_checked[payload_offset + 24] == 0x00 && packet_checked[payload_offset + 22] != 0x83)
    {
      if ((int)(packet_checked[payload_offset + 3]) < 26)
      {
        if (b_counter == 1)
        {
          fp = fopen("/home/ydh6226/Desktop/BlackList.txt", "w");
          b_counter++;
        }
        else if (b_counter != 1)
          fp = fopen("/home/ydh6226/Desktop/BlackList.txt", "a");
      }
      else
      {
        ...
        else
        {
          ...
          if (w_counter == 1)
          {
            fp = fopen("/home/ydh6226/Desktop/WhiteList.txt", "w");
            w_counter++;
          }
          else if (w_counter != 1)
            fp = fopen("/home/ydh6226/Desktop/WhiteList.txt", "a");
        }
      }
    }
    else
    ...
    // printf("Length: %s num : %d\n", (int)(packet_checked[payload_offset + 3]), counter);
    fprintf(fp, "==========packet count: %d num: %d==========\n\n", counter, all_count);
    counter++;
    fprintf(fp, "<<TPKT>>\n\n");
    ...
    else
    {
      ...
      else if (packet_checked[payload_offset + 22] == 0x43)
      {
        ...
        if (packet_checked[payload_offset + 23] == 0x03)
        {
          fprintf(fp, "Get block info (3)\n");
        }
        else if (packet_checked[payload_offset + 23] == 0x02)
        {
          fprintf(fp, "List blocks of type (2)\n");
        }
        else if (packet_checked[payload_offset + 23] == 0x01)
        {
          fprintf(fp, "List blocks (1)\n");
        }
        else
        {
          fprintf(fp, "undefined subfunction\n");
        }
        //ydh_end
      }
      ...
      else if ((packet_checked[payload_offset + 23] == 0x02)&&((packet_checked[payload_offset+22]&0x0f)!=0x03))
      {
      ...
      //ydh_start
      if (packet_checked[payload_offset + 22] == 0x43)
      { //Request block function
        if (packet_checked[payload_offset + 23] == 0x03)
        {
          fprintf(fp, "Block type : %c%c\n", packet_checked[payload_offset + 29], packet_checked[payload_offset + 30]);
          fprintf(fp, "Block number : ");
          for (int i = 31; i < 36; i++)
          {
            fprintf(fp, "%c", packet_checked[payload_offset + i]);
          }
          fprintf(fp, "\n");
          fprintf(fp, "Filesystem : %c\n", packet_checked[payload_offset + 36]);
        }
        else if (packet_checked[payload_offset + 23] == 0x02)
        {
          fprintf(fp, "Block type : %c%c\n", packet_checked[payload_offset + 29], packet_checked[payload_offset + 30]);
      ...
      { //Response block function
        ...
        {
          data_len = packet_checked[payload_offset + 31];
          data_len = ((data_len << 8) + packet_checked[payload_offset + 32])/4;
          for (int i = 0; i < data_len; i++)
          {
            i=i*4;
            fprintf(fp, "Item [%d]\n", i/4+1);
            fprintf(fp, "block number : %d\n", packet_checked[payload_offset + 34 + i]);
            fprintf(fp, "Block flags : 0x%.2x\n", packet_checked[payload_offset + 35 + i]);
            fprintf(fp, "Block language : 0x%.2x\n", packet_checked[payload_offset + 36 + i]);
            i=i/4;
          }
        }
        else if (packet_checked[payload_offset + 23] == 0x01)
        {
          ...
          for (int i = 0; i<data_len; i++)
          {
            i=i*4;
            fprintf(fp, "Item [%d]\n", i/4 + 1);
            fprintf(fp, "Block type : %c%c\n", packet_checked[payload_offset+33 + i], packet_checked[payload_offset+34 + i]);
            fprintf(fp, "BLock count : 0x%.2x\n", (packet_checked[payload_offset+35 + i] << 8) + packet_checked[payload_offset+36 + i]);
            i=i/4;
          }
        }
      }
    ...
    int s7_all = s7_counter.counter_reqSZL_req + s7_counter.counter_readClock_req + s7_counter.counter_msgService_req + s7_counter.counter_reqSZL_res + s7_counter.counter_readClock_res + s7_counter.counter_msgService_res + 1;
  ...
  if (extcap_dumper && ((extcap_packet_filter == (u_int16_t)-1) || (p.app_protocol == extcap_packet_filter) || (p.master_protocol == extcap_packet_filter)))
  {
