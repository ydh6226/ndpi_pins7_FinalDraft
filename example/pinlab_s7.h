...
void bit_analysis(uint8_t *s7packet,unsigned long packet_len,uint16_t id, uint16_t index,FILE *fp){
    ...
    switch(s7key){
        ...
        //woo_add_start
        case 0x00130000:
            for(seq = 12; seq<packet_len; seq+=36){
                fprintf(fp, "SZL data tree (list count no. %d)\n", no);
                data = packet_checked[seq];
                data = (data<<8)+packet_checked[seq+1];
                fprintf(fp, "Index : 0x%.4x\n", data);
                fprintf(fp, "Code (Memory type): 0x%.2x%.2x\n", packet_checked[seq+2], packet_checked[seq+3]);
                fprintf(fp, "Size (Total size of the selected memory, total of area 1 and area 2): 0x%.2x%.2x%.2x%.2x\n", packet_checked[seq+4], packet_checked[seq+5], packet_checked[seq+6], packet_checked[seq+7]);
                fprintf(fp, "Mode (Logical mode of the memory): 0x%.2x%.2x\n", packet_checked[seq+8], packet_checked[seq+9]);
                fprintf(fp, "Volatile memory area: %s\n",packet_checked[seq+9]%16%2==0?"False":"True");
                fprintf(fp, "Non-volatile memory area: %s\n",packet_checked[seq+9]%16/2%2==0?"False":"True");
                fprintf(fp, "Mixed memory area: %s\n",packet_checked[seq+9]%16/4%2==0?"False":"True");
                fprintf(fp, "Code and data separate (for work memory): %s\n",packet_checked[seq+9]%16/8%2==0?"False":"True");
                fprintf(fp, "Code and data together (for work memory): %s\n",packet_checked[seq+9]/16%2==0?"False":"True");
                fprintf(fp, "Granu: 0x%.2x%.2x\n",packet_checked[seq+10], packet_checked[seq+11]);
                fprintf(fp, "ber1 (Size of the volatile memory area in bytes): 0x%.2x%.2x%.2x%.2x\n",packet_checked[seq+12], packet_checked[seq+13], packet_checked[seq+14], packet_checked[seq+15]);
                fprintf(fp, "belegt1 (Size of the volatile memory area being used): 0x%.2x%.2x%.2x%.2x\n",packet_checked[seq+16], packet_checked[seq+17], packet_checked[seq+18], packet_checked[seq+19]);
                fprintf(fp, "block1 (Largest free block in the volatile memory area): 0x%.2x%.2x%.2x%.2x\n",packet_checked[seq+20], packet_checked[seq+21], packet_checked[seq+22], packet_checked[seq+23]);
                fprintf(fp, "ber2 (Size of the non-volatile memory area in bytes): 0x%.2x%.2x%.2x%.2x\n",packet_checked[seq+24], packet_checked[seq+25], packet_checked[seq+26], packet_checked[seq+27]);
                fprintf(fp, "belegt2 (Size of the non-volatile memory area being used): 0x%.2x%.2x%.2x%.2x\n",packet_checked[seq+28], packet_checked[seq+29], packet_checked[seq+30], packet_checked[seq+31]);
                fprintf(fp, "block2 (Largest free block in the non-volatile memory area): 0x%.2x%.2x%.2x%.2x\n\n",packet_checked[seq+32], packet_checked[seq+33], packet_checked[seq+34], packet_checked[seq+35]);
                no++;
            }
            break;
        //woo_add_end
        ...
        case 0x01310004:
            for(seq = 12; seq<packet_len; seq+=40){
                ...
                fprintf(fp, "funkt_0: 0x%.2x\n", packet_checked[seq+2]);
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+2]%16%2==0?"False":"True");
                fprintf(fp, "Directory (hierarchy 1): %s\n",packet_checked[seq+2]%16/2%2==0?"False":"True");
                fprintf(fp, "Directory (hierarchy 2): %s\n",packet_checked[seq+2]%16/4%2==0?"False":"True");
                fprintf(fp, "Directory (hierarchy 3): %s\n",packet_checked[seq+2]%16/8%2==0?"False":"True");
                fprintf(fp, "Copy: %s\n",packet_checked[seq+2]/16%2==0?"False":"True");
                fprintf(fp, "Chain (list): %s\n",packet_checked[seq+2]/16/2%2==0?"False":"True");
                fprintf(fp, "Chain (all copied): %s\n",packet_checked[seq+2]/16/4%2==0?"False":"True");
                fprintf(fp, "Delete (list): %s\n",packet_checked[seq+2]/16/8%2==0?"False":"True");
                fprintf(fp, "\n");

                fprintf(fp, "funkt_1: 0x%.2x\n", packet_checked[seq+3]);
                fprintf(fp, "Upload on PG: %s\n",packet_checked[seq+3]%16%2==0?"False":"True");
                fprintf(fp, "Assign parameters when chaining: %s\n",packet_checked[seq+3]%16/2%2==0?"False":"True");
                fprintf(fp, "LOAD function when exchanging data with CFBs: %s\n",packet_checked[seq+3]%16/4%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+3]%16/8%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+3]/16%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+3]/16/2%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+3]/16/4%2==0?"False":"True");
                fprintf(fp, "Delete *.*: %s\n",packet_checked[seq+3]/16/8%2==0?"False":"True");
                ...
                fprintf(fp, "funkt_2: 0x%.2x\n", packet_checked[seq+4]);
                fprintf(fp, "Load user program (RAM): %s\n",packet_checked[seq+4]%16%2==0?"False":"True");
                fprintf(fp, "Load user program (EPROM): %s\n",packet_checked[seq+4]%16/2%2==0?"False":"True");
                fprintf(fp, "Save user program (RAM): %s\n",packet_checked[seq+4]%16/4%2==0?"False":"True");
                fprintf(fp, "Save user program (EPROM): %s\n",packet_checked[seq+4]%16/8%2==0?"False":"True");
                fprintf(fp, "Save user program (all): %s\n",packet_checked[seq+4]/16%2==0?"False":"True");
                fprintf(fp, "Comopress (external): %s\n",packet_checked[seq+4]/16/2%2==0?"False":"True");
                fprintf(fp, "Firmware update (using communication): %s\n",packet_checked[seq+4]/16/4%2==0?"False":"True");
                fprintf(fp, "Set RAM memory mode: %s\n",packet_checked[seq+4]/16/8%2==0?"False":"True");
                fprintf(fp, "\n");

                fprintf(fp, "funkt_3: 0x%.2x\n", packet_checked[seq+5]);
                fprintf(fp, "Set EPROM memory mode: %s\n",packet_checked[seq+5]%16%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+5]%16/2%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+5]%16/4%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+5]%16/8%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+5]/16%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+5]/16/2%2==0?"False":"True");
                fprintf(fp, "Assign parameters to newly plugged in modules: %s\n",packet_checked[seq+5]/16/4%2==0?"False":"True");
                fprintf(fp, "Assign parameters when evaluating memory card: %s\n",packet_checked[seq+5]/16/8%2==0?"False":"True");
                fprintf(fp, "\n");

                fprintf(fp, "funkt_4: 0x%.2x\n", packet_checked[seq+6]);
                fprintf(fp, "Assign parameters when loading user program: %s\n",packet_checked[seq+6]%16%2==0?"False":"True");
                fprintf(fp, "Assign parameters in complete restart: %s\n",packet_checked[seq+6]%16/2%2==0?"False":"True");
                fprintf(fp, "Assign parameters in restrat: %s\n",packet_checked[seq+6]%16/4%2==0?"False":"True");
                fprintf(fp, "Compress (SFC25 COMPRESS): %s\n",packet_checked[seq+6]%16/8%2==0?"False":"True");
                fprintf(fp, "Evaluate memory card after switch setting: %s\n",packet_checked[seq+6]/16%2==0?"False":"True");
                fprintf(fp, "Firmware update using memory card: %s\n",packet_checked[seq+6]/16/2%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+6]/16/4%2==0?"False":"True");
                fprintf(fp, "Reserved: %s\n",packet_checked[seq+6]/16/8%2==0?"False":"True");
                fprintf(fp, "\n");

                fprintf(fp, "funkt_5 (Reserved): 0x%.2x\n", packet_checked[seq+7]);
                fprintf(fp, "\n");

                fprintf(fp, "funkt_6 (Reserved): 0x%.2x\n", packet_checked[seq+8]);
                fprintf(fp, "\n");

                fprintf(fp, "funkt_7 (Reserved): 0x%.2x\n", packet_checked[seq+9]);
                fprintf(fp, "\n");

                fprintf(fp, "kop (Maximum number of copied blocks): 0x%.2x\n", packet_checked[seq+10]);
                fprintf(fp, "del (Maximum number of uninterruptable, deletable blocks): 0x%.2x\n", packet_checked[seq+11]);
                fprintf(fp, "kett (Maximum number of blocks chained in one job): 0x%.2x\n", packet_checked[seq+12]);
                fprintf(fp, "hoch (Miximum number of simultaneous upload procedures): 0x%.2x\n", packet_checked[seq+13]);
                fprintf(fp, "ver (Maximum size (in bytes) of shiftable blocks in RUN): 0x%.2x\n", packet_checked[seq+14]);
                fprintf(fp, "res (Reserved): 0x");
                for(int i = 15;i<40;i++){
                    fprintf(fp, "%.2x", packet_checked[seq+i]);
                }fprintf(fp, "\n");
                no++;
        ...
        case 0x01320002:
            for(seq=12;seq<packet_len;seq+=40){
                fprintf(fp, "SZL data tree (list count no. %d)\n",no);
                fprintf(fp, "Index: 0x%.2x%.2x\n",packet_checked[seq], packet_checked[seq+1]);
                fprintf(fp, "anz (Number of initialized test and installation jobs): 0x%.2x%.2x\n", packet_checked[seq+2],packet_checked[seq+3]);
                fprintf(fp, "res (Reserved) : 0x");
                for(sim=4; sim<40; sim++){
                    fprintf(fp, "%.2x", packet_checked[seq+sim]);
                }
            }
            fprintf(fp, "\n");
            break;
